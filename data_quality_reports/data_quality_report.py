"""Functions for data quality reports."""
...
def dqr(df: pd.DataFrame, save_as_csv: bool = False) -> pd.DataFrame:
    """Generate basic Data Quality Report for the DataFrame provided.
    ...
    continuous_data: pd.DataFrame
        DataFrame containing the statistical summary and other data quality indicators.
    ...
    continuous_data = data_types[
        (data_types["data_type"] != "object") | (data_types["data_type"] == "datetime64[ns]")
    ].copy()
    categorical_data = data_types[
        (data_types["data_type"] == "object") & (data_types["data_type"] != "datetime64[ns]")
    ].copy()

    # Update Continuous Data Table
    continuous_data["min"] = [
        round(df[ind].min(), 2) if continuous_data.loc[ind, "data_type"] != "<M8[ns]" else df[ind].min()
        for ind in continuous_data.index
    ]
    continuous_data["1st qrt"] = [
        round(df[ind].quantile([0.25]).values[0], 2)
        if continuous_data.loc[ind, "data_type"] != "<M8[ns]"
        else df[ind].quantile([0.25]).values[0]
        for ind in continuous_data.index
    ]
    continuous_data["mean"] = [
        round(df[ind].mean(), 2) if continuous_data.loc[ind, "data_type"] != "<M8[ns]" else df[ind].mean()
        for ind in continuous_data.index
    ]
    continuous_data["median"] = [
        round(df[ind].median(), 2) if continuous_data.loc[ind, "data_type"] != "<M8[ns]" else df[ind].median()
        for ind in continuous_data.index
    ]
    continuous_data["3rd qrt"] = [
        round(df[ind].quantile([0.75]).values[0], 2)
        if continuous_data.loc[ind, "data_type"] != "<M8[ns]"
        else df[ind].quantile([0.75]).values[0]
        for ind in continuous_data.index
    ]
    continuous_data["max"] = [
        round(df[ind].max(), 2) if continuous_data.loc[ind, "data_type"] != "<M8[ns]" else df[ind].max()
        for ind in continuous_data.index
    ]
    continuous_data["std dev"] = [
        round(df[ind].std(), 2) if continuous_data.loc[ind, "data_type"] != "<M8[ns]" else df[ind].std()
        for ind in continuous_data.index
    ]
    ...
    mode_2 = [
        df[ind].value_counts().index[1] if categorical_data.loc[ind, "cardinality"] > 1 else np.nan
        for ind in categorical_data.index
    ]
    mode_freq_2 = [
        df[ind].value_counts().iloc[1] if categorical_data.loc[ind, "cardinality"] > 1 else np.nan
        for ind in categorical_data.index
    ]
